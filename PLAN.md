...
- ✅ Settings UI에서 서비스별 "지금 동기화" / "전체 동기화" 버튼 제공
- ✅ 동기화 결과/실패 이력 UI 및 로그 가시화
- ✅ 증분 동기화 안정성 1차 보강 (429/5xx 백오프 재시도, 라벨 기반 상태 매핑 보완)
- ✅ 증분 동기화 안정성 2차 보강 (대량 계정 분산 처리, 계정 단위 백오프 정책 고도화)
- ✅ Gmail 라벨/상태 매핑 정교화 (Gmail 라벨 SOT: `INBOX`/`SENT`/`UNREAD` 기준)
